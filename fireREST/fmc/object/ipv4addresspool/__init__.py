...
class Ipv4AddressPool(Resource):
    ...
    **Operation IDs:**
    ...
    - `getAllIPv4AddressPool` (GET (list))
    - `getIPv4AddressPool` (GET)
    - `createIPv4AddressPool` (CREATE)
    - `updateIPv4AddressPool` (UPDATE)
    - `deleteIPv4AddressPool` (DELETE)

    **Query parameters:**

    - `overrideTargetId` (string, optional): Retrieves the override(s) associated with the IPv4 Address Pool object on given target ID.
